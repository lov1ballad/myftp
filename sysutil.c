...
int tcp_client(int port)
{
	...
	address.sin_addr.s_addr = INADDR_ANY;//采用主机上任一网卡
...
static struct timeval s_cur_time;//定义当前时间结构体
unsigned long long get_time_sec()
{
	if(gettimeofday(&s_cur_time, NULL) < 0)//获取系统当前时间
...
void nano_sleep(double sleep_time) 
{
	...
	double decimal = sleep_time - (double)sec;//小数部分
