...
void Test_Parseconf()
{
	parseconf_load_file("myftp.conf");
...
static struct hash *s_ip_count_hash;//IP与其连接数的映射
static struct hash *s_pid_ip_hash;//PID与ip的映射

static void check_limit(session_t *sess);
static void handle_sigchld(int sig);

unsigned int hash_func(unsigned int buket_size, void *key);
unsigned int handle_ip_count(void *ip);//计算该ip连接的个数
...
int main(int argc, char *argv[])
{
	...
	*/
    ...
    while(1)
    {
        ...
        else//父进程一直在while循环里，不停接收连接、创建进程、等待连接
        {
			//增加pid跟ip的映射
...
static void handle_sigchld(int sig)
{
	...
	//减少每ip的连接数
	pid_t pid;
	while((pid = waitpid(-1, NULL, WNOHANG)) > 0)//因为waitpid的返回值为pid，无法通过pid获取count，因此维护第二个哈希表
	{
		unsigned int *ip = (unsigned int *)hash_lookup_entry(s_pid_ip_hash, &pid, sizeof(pid));
		if(ip == NULL)
			continue;
		drop_ip_count(ip);//count个数减一
